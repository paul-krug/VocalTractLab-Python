...
formants = []
for state in np.array( random_states ).T:
	#print( np.array( state ).shape )
	df_transfer = vtl.get_transfer_function( np.array( [state] ), n_spectrum_samples )
	peaks, _ = find_peaks( df_transfer.loc[ 0, 'magnitude' ][ : 500], distance = 5 )
	peaks.sort()
	#print(peaks)
	if len(peaks) >= 2:
		formants.append( [ peaks[0] * 44100/n_spectrum_samples, peaks[1] * 44100/n_spectrum_samples ] )
	else:
...
df_transfer.to_csv( 'transfer_random_uniform_{}.txt'.format(n_samples) )
